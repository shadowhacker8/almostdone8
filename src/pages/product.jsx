  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      try {
        ...
        // --- Start: Meta Tag Management (Aggressive Clear & Add) ---
        // Clear all previous dynamic meta and link tags before adding new ones
        ...
        // Set document title
        document.title = data.meta?.title || `${data.title} - HappyDeel`;

        const currentUrl = window.location.href; 
        const ogTitle = data.meta?.ogTitle || data.meta?.title || `${data.title} - HappyDeel`;
        const ogDescription = data.meta?.ogDescription || data.meta?.description || `Discover ${data.title} on HappyDeel. ${data.description}.`;
        const ogImage = data.meta?.ogImage || (data.images && data.images[0] ? new URL(data.images[0], window.location.origin).href : '');
        const twitterTitle = data.meta?.twitterTitle || ogTitle;
        const twitterDescription = data.meta?.twitterDescription || ogDescription;
        const twitterImage = data.meta?.twitterImage || ogImage;

        // Add Meta Description
        const descTag = document.createElement('meta');
        descTag.setAttribute('name', 'description');
        descTag.setAttribute('content', data.meta?.description || `Discover ${data.title} on HappyDeel. ${data.description}.`);
        document.head.appendChild(descTag);

        // Add Meta Keywords
        const keywordsTag = document.createElement('meta');
        keywordsTag.setAttribute('name', 'keywords');
        keywordsTag.setAttribute('content', data.meta?.keywords || `${data.title}, HappyDeel, ecommerce, online shopping`);
    ...
    return () => {
      ...
      // --- Start: Meta Tag Cleanup on Unmount (Aggressive Clear) ---
      document.querySelectorAll(`meta[name="description"],
                                meta[name="keywords"],
                                meta[property^="og:"],
                                meta[name^="twitter:"],
      ...
      document.title = 'HappyDeel'; // Reset title
      // --- End: Meta Tag Cleanup on Unmount --- 
    };
  }, [slug, navigate]);
